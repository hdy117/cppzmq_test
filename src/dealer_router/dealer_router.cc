...
void dealer_client(zmq::context_t &context, int client_id) {
  try {
    ...
    // 发送多个请求
    for (int i = 0; i < 3; ++i) {
      // 构造请求消息, [identity][空帧][消息内容], identity

      // empty frame
      client_socket.send(zmq::message_t(), zmq::send_flags::sndmore);
      // msg content
      std::string request = "this is client id: " + std::to_string(client_id);
      client_socket.send(zmq::buffer(request), zmq::send_flags::none);
      LOG(INFO) << "client id: " << client_id << " send: " << request
                << std::endl;

      // 等待服务器响应
      // zmq::message_t identity;
      // client_socket.recv(identity, zmq::recv_flags::none);
      zmq::message_t empty_frame;
      client_socket.recv(empty_frame, zmq::recv_flags::none);
      zmq::message_t response;
      client_socket.recv(response, zmq::recv_flags::none);
      LOG(INFO) << "client id: " << client_id << " got response: "
                << std::string(static_cast<char *>(response.data()),
                               response.size())
                << std::endl;

      // 模拟工作负载
      std::this_thread::sleep_for(
          std::chrono::milliseconds(100 * (client_id + 1)));
    }
...
void router_server(zmq::context_t &context) {
  try {
    ...
    while (!stop_server_flag.load()) {
      ...
      // 接收client标识
      zmq::message_t identity;
      auto result = server_socket.recv(identity, zmq::recv_flags::dontwait);

      if (result.has_value() && result.value() > 0) {
        // recv empty frame
        zmq::message_t empty_frame;
        server_socket.recv(empty_frame, zmq::recv_flags::none);
        ...
        LOG(INFO) << "server got request from client id: " << client_id
                  << " content: " << request_str << std::endl;

        // 发送响应：[client标识][空帧][响应内容]
        // 发送client标识
        server_socket.send(identity, zmq::send_flags::sndmore);

        // 发送空帧
        server_socket.send(zmq::message_t(), zmq::send_flags::sndmore);

        // 发送响应内容
        std::string response = "server processed " + request_str;
        server_socket.send(zmq::buffer(response), zmq::send_flags::none);
      }
